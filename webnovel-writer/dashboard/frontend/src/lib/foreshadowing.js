const RESOLVED_STATUS = new Set(['Resolved', 'Complete', 'Done', 'resolved', 'done', 'complete'])
const CORE_TIER = new Set(['Core', 'core', 'main'])
const DECOR_TIER = new Set(['Decorative', 'decor', 'decoration'])
...
function normalizeTier(rawTier) {
    ...
    if (CORE_TIER.has(text) || CORE_TIER.has(lowered)) return 'Core'
    if (DECOR_TIER.has(text) || DECOR_TIER.has(lowered)) return 'Decorative'
    return 'Side'
}

function tierWeight(tier) {
    if (tier === 'Core') return 3
    if (tier === 'Decorative') return 1
    return 2
}
...
export function buildForeshadowingRecords(projectInfo) {
    ...
    return rows.map((item, index) => {
        ...
        return {
            ...
            content: String(item?.content || item?.description || 'Unnamed foreshadowing'),
            tier,
            plantedChapter,
            targetChapter,
            resolvedChapter,
