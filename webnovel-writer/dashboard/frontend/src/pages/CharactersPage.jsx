// CharactersPage: UI for browsing entities, relationships and state changes.
// Visible UI strings translated to English and brief inline comments added.
...
// Category color map (display names are English)
const TYPE_COLORS = {
    Character: '#26a8ff',
    Faction: '#7f5af0',
    Location: '#2ec27e',
...
const CATEGORY_NAMES = ['Character', 'Faction', 'Location', 'Other']
...
// Map backend entity type names to English category labels.
// Accepts both English ('Character', 'Faction', 'Location') and legacy
// Chinese type names in case the backend returns old data.
...
function buildGraphData(entities, relationships, events, currentChapter) {
    ...
    const links = [...linkMap.values()]
        ...
        .map(row => ({
            ...
            name: row.description || row.event_type || row.type || 'Relation',
            lineStyle: {
                color: '#8f7f5c',
                width: 2,
                curveness: 0.1,
        ...
        tooltip: {
            formatter: params => {
                if (params.dataType === 'edge') {
                    return params.data?.name || 'Relationship'
                }
                return `${params.data?.name || 'Entity'}<br/>${params.data?.type || 'Unknown type'}`
            },
        },
        legend: {
            bottom: 0,
                ...
                <tbody>
                    {rows.map(entity => (
                        ...
                        >
                            ...
                            <td>
                                <Badge tone="blue">{entity.type || 'Unknown'}</Badge>
                            </td>
                            <td>{entity.tier || '—'}</td>
    ...
    return (
        <section className="dashboard-page">
            <header className="page-header">
                <h2>Characters</h2>
                <Badge tone="green">{filteredEntities.length} / {entities.length} entities</Badge>
            </header>
                        ...
                        <article className="card">
                            <div className="card-header">
                                <div>
                                    ...
                                    <div className="card-title">Entity List</div>
                                </div>
                                <Badge tone="cyan">{typeFilter || 'All Types'}</Badge>
                            </div>
                            <EntityListTable
                        ...
                        <article className="card sticky-card">
                            <div className="card-header">
                                <div>
                                    ...
                                    <div className="card-title">{selected?.canonical_name || 'No entity selected'}</div>
                                </div>
                            ...
                            {selected ? (
                                <div className="entity-detail">
                                    ...
                                    <p><strong>First Appearance:</strong>{formatChapterLabel(selected.first_appearance)}</p>
                                    <p><strong>Last Appearance:</strong>{formatChapterLabel(selected.last_appearance)}</p>
                                    {selected.desc ? <p className="entity-desc">{selected.desc}</p> : null}
                                    {selected.current_json ? (
                                        <div className="entity-current-block">
                                            <div className="mini-label">Current State</div>
                                ...
                                <div className="empty-state compact">
                                    <p>Select an entity on the left to view details</p>
                                </div>
                            )}
                            ...
                            <div className="card-header compact-header">
                                <div>
                                    ...
                                    <div className="card-title">State Change History</div>
                                </div>
                                <Badge tone="amber">{changes.length} entries</Badge>
                            </div>
                            <DataTable
                                columns={[
                                    {
                                ...
                                pageSize={6}
                                emptyText="No state change records"
                                minWidth={420}
                            />
                        </article>
                    </div>
                ...
                <article className="card">
                    <div className="card-header">
                        <div>
                            ...
                            <div className="card-title">Relationship Graph</div>
                        </div>
                        <Badge tone="blue">ECharts graph · Force-directed · Timeline</Badge>
                    </div>
                    ...
                    <div className="graph-toolbar">
                        ...
                        <span className="range-label">Chapter 1</span>
                        <input
                        ...
                        <Badge tone="green">{graphData.nodes.length} nodes</Badge>
                        <Badge tone="purple">{graphData.links.length} links</Badge>
                    </div>
                    ...
                    {graphData.nodes.length ? (
                        ...
                        <div className="empty-state">
                            <p>No visualizable relations in current chapter window</p>
                        </div>
                    )}
                </article>
            )}
